...
import           Luna.Studio.React.Model.Node.SidebarNode  (countProjectionPorts, hasPort, isInputSidebar, ports)
...
localRemovePort (OutPortRef' (OutPortRef nid pid@(Projection pos _))) = do
    ...
    flip (maybe (return False)) mayNode $ \node ->
        if (not $ isInputSidebar node) || (not $ hasPort (OutPortId pid) node) || countProjectionPorts node == 1
            then return False
            else do
