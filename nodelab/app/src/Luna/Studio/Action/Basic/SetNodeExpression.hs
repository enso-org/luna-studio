...
import           Empire.API.Data.Node                (NodeId, NodeType (ExpressionNode), nodeType)
import qualified Luna.Studio.Action.Batch            as Batch
import           Luna.Studio.Action.Command          (Command)
import qualified Luna.Studio.Action.State.Graph      as Graph
import qualified Luna.Studio.Action.State.NodeEditor as NodeEditor
import           Luna.Studio.Prelude
import           Luna.Studio.React.Model.Node        (expression)
...
localSetNodeExpression nid update = do
    mayType <- (fmap . fmap) (view nodeType) $ Graph.getNode nid
    case mayType of
        Just (ExpressionNode _) -> do
            Graph.modifyNode      nid $ nodeType   .~ ExpressionNode update
            NodeEditor.modifyNode nid $ expression .= update
            return True
        _                   -> return False
