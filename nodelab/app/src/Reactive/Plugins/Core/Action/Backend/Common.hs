...
whenOk (Response.Response _ _ _ (Response.Ok    res)) handler = handler res
whenOk (Response.Response _ _ _ (Response.Error _  )) _       = return ()

handleResponse :: (Topic.MessageTopic (Response.Response req inv res), JSON.ToJSON req) => Response.Response req inv res -> (req -> res -> Command State ()) -> Command State ()
handleResponse resp@(Response.Response uuid req inv status) success =
