...
import           Object.Widget                   (ClickHandler, IsDisplayObject, KeyDownHandler, UIHandlers, WidgetId, dblClick,
                                                  fromWidgetId, keyDown, widgetSize)
...
labeledEditableResize :: IsDisplayObject a => WidgetId -> Size -> a -> Command UIRegistry.State ()
labeledEditableResize widgetId size model = do
    ...
    UICmd.resize tbId $ Size (Vector2 sx ty)
    UICmd.moveX tbId tx
