{-# LANGUAGE MultiWayIf #-}
module Node.Editor.Action.State.Model.Connection where
...
import           Node.Editor.Action.State.NodeEditor         (getConnection, getNode, getNodeEditor, getPosConnection, getPosConnections)
import           Node.Editor.Data.Geometry                   (closestPointOnLine, closestPointOnLineParam, doesSegmentsIntersects)
import           Luna.Prelude
import           Node.Editor.React.Model.Connection          (Connection (Connection), ConnectionId, HalfConnection (HalfConnection),
                                                              connectionId, connectionMode, containsNode, halfConnectionMode)
import qualified Node.Editor.React.Model.Connection          as Model
import           Node.Editor.React.Model.Constants           (nodeRadius)
import           Node.Editor.React.Model.Node                (NodeLoc)
import           Node.Editor.React.Model.Node.ExpressionNode (ExpressionNode, isCollapsed, nodeLoc, position)
import           Node.Editor.React.Model.NodeEditor          (toPosConnection)
