module Node.Editor.Action.Basic.UpdateCollaboration where
...
import           Node.Editor.Action.State.NodeEditor         (getSelectedNodes, modifyExpressionNodes_)
import           Luna.Prelude
import           Node.Editor.React.Model.Node.ExpressionNode (modify, nodeLoc, touch)
import qualified Node.Editor.React.Model.Node.ExpressionNode as Node
import           Node.Editor.State.Collaboration             (Client (Client), ColorId (ColorId), colorId, knownClients, lastSeen,
                                                              unColorId)
import           Node.Editor.State.Global                    (State, collaboration, lastEventTimestamp)
