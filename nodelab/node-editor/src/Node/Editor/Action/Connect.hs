...
module Node.Editor.Action.Connect
...
import           Node.Editor.Action.Basic                    (connect, localRemovePort, removeConnection, updateAllPortsSelfVisibility)
import qualified Node.Editor.Action.Batch                    as Batch
import           Node.Editor.Action.Command                  (Command)
import           Node.Editor.Action.Node.Drag                (startNodeDrag)
import           Node.Editor.Action.State.Action             (beginActionWithKey, continueActionWithKey, removeActionFromState,
                                                              updateActionWithKey)
import           Node.Editor.Action.State.Model              (createHalfConnectionModel, createHalfConnectionModel')
import           Node.Editor.Action.State.NodeEditor         (getConnection, getNode, modifyNodeEditor)
...
import           Node.Editor.React.Model.Connection          (ConnectionId, toValidEmpireConnection)
import qualified Node.Editor.React.Model.Connection          as Connection
import           Node.Editor.React.Model.Node                (Node (Expression))
import           Node.Editor.React.Model.Node.ExpressionNode (isCollapsed)
import qualified Node.Editor.React.Model.NodeEditor          as NodeEditor
import           Node.Editor.State.Action                    (Action (begin, continue, end, update), Connect (Connect), Mode (Click, Drag),
                                                              connectAction, connectIsPortPhantom, connectMode, connectSnappedPort,
                                                              connectSourcePort, connectStartPos)
import           Node.Editor.State.Global                    (State, actions, currentConnectAction)
