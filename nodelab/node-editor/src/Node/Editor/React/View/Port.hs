...
module Node.Editor.React.View.Port where
...
import           Node.Editor.React.Model.Constants (gridSize, lineHeight, nodeRadius, nodeRadius')
import           Node.Editor.React.Model.Node      (NodeLoc)
--import           Node.Editor.React.Model.Port (Port (..))
import           Node.Editor.React.Model.Port      (AnyPort, AnyPortId (InPortId', OutPortId'), InPortIndex (Self),
...
port :: ReactView (Ref App, NodeLoc, Int, Bool, AnyPort)
...
port_ :: Ref App -> NodeLoc -> AnyPort -> Int -> Bool -> ReactElementM ViewEventHandler ()
