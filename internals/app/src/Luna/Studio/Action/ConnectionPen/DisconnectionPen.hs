...
instance Action (Command State) PenDisconnect where
    begin    = undefined --beginActionWithKey    penDisconnectAction
    continue = undefined --continueActionWithKey penDisconnectAction
    update   = undefined --updateActionWithKey   penDisconnectAction
    end _    = undefined --removeActionFromState penDisconnectAction


startDisconnecting :: MouseEvent -> Timestamp -> Command State ()
startDisconnecting evt timestamp = undefined --do
...
checkAndUpdateRestriction conn = undefined -- continue $ \state -> withJust (state ^. penDisconnectLastVisitedNode) $ \lastNode -> do
...
handleSegment seg@(_, segEnd) = undefined --do
...
disconnectProcessSegment seg =undefined -- do
...
disconnectMove evt timestamp state =undefined -- do
    -- pos <- workspacePosition evt
...
stopDisconnecting state =undefined -- do
