...
separateSubgraph nodeIds = undefined --do
...
addConnection conn = undefined -- modifyGraph $ connectionsMap . at (conn ^. connectionId) ?~ conn
...
getConnectionsBetweenNodes nid1 nid2 = undefined
...
getConnectionsContainingNode nid = undefined --filter (containsNode nid) <$> getConnections

getConnectionsContainingNodes :: [NodeId] -> Command State [Connection]
getConnectionsContainingNodes nodeIds = undefined --filter containsNode' <$> getConnections where
...
getConnectionsContainingPortRef portRef = undefined --filter (containsPortRef portRef) <$> getConnections

getConnectionsToNode :: NodeId -> Command State [Connection]
getConnectionsToNode nid = undefined --filter (\conn -> conn ^. dstNodeId == nid) <$> getConnections
