...
removePort portRef = undefined --whenM (localRemovePort portRef) $ Batch.removePort portRef

localRemovePort :: AnyPortRef -> Command State Bool
localRemovePort (OutPortRef' (OutPortRef nid pid@(Projection pos))) = undefined --do
    -- mayNode      <- getNode nid
